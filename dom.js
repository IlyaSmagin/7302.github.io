...
function printResult(result, elapsedTime) {
  const resultHalf = document.getElementById('right-half');
...
function clearResults() {
  const elements = document.getElementById('right-half').childNodes;
...
function processForm(form) {
  const module = Modules[form.select.options[form.select.selectedIndex].value];
  ...
  let retVal = module.func.apply(this, args);
  const elapsedTime = performance.now() - timeBeforeCall;
...
function selectOnChange(select) {
  const module = Modules[select.options[select.selectedIndex].value];
  ...
  // И создаем новые
  for (let i = 0; i < module.reqFields.length; i++) {
    const field = module.reqFields[i];
    const fieldDiv = document.createElement('div');
    fieldDiv.setAttribute('class', 'arg');
    const divContent = document.createTextNode(field.caption);
...
function validateOpt(option) {
  ...
  const regexType = option.classList.item(0);
  if (regexps[regexType] === undefined)
    debugger;
  const validated = regexps[regexType].test(option.value);
  if (validated)
...
function formatSelect(radio) {
  ...
  const elements = select.childNodes;
