...
function printResult(result, elapsedTime) {
  ...
  if (resultHalf.lastChild !== null) resultHalf.lastChild.classList.remove("last");
...
function processForm(form) {
  ...
  if (module.func === undefined) return false; // Проверяем поля

  var validated = true;
  var elements = document.querySelectorAll("input[type=text]");

  for (var i = 0; i < elements.length; i++) {
    if (!validateOpt(elements[i])) validated = false;
  }

  if (!validated) return false; // Формируем аргументы и вызываем функцию
  ...
  var elapsedTime = performance.now() - timeBeforeCall; // Формируем результат

  if (typeof retVal !== "string") {
    if (module.formatter !== undefined) retVal = module.formatter(retVal);else if (module.returnCodes !== undefined) retVal = module.returnCodes[retVal];
  } // Выводим
...
function selectOnChange(select) {
  ...
  if (module === undefined || module.reqFields === undefined) debugger; // Удаляем старые поля
  ...
  if (module.comment !== undefined) comment.appendChild(document.createTextNode(module.comment));
}
...
function validateOpt(option) {
  ...
  if (regexps[regexType] === undefined) debugger;
  var validated = regexps[regexType].test(option.value);
  if (validated) option.classList.remove("invalidated");else option.classList.add("invalidated");
  return validated;
}

function formatSelect(radio) {
  var select = document.getElementById("select");
  var elements = select.childNodes; // Только переключаем отображение
  ...
  for (var _moduleName in Modules) {
    ...
    if (_module.func !== undefined && !document.getElementById("type1").checked) fieldOpt.innerHTML = _moduleName;else fieldOpt.innerHTML = _module.description;
